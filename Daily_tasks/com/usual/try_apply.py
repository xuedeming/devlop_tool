...
class Mysql:
    ...
    def query(self):
        ...
        # 查询用户id
        query_user = 'SELECT id,school_id from sys_user  WHERE mobile= %s and school_id = 5'
        # 用户新增试用学校老师权限
        ...
        # 修改处理状态
        status = 'update try_apply set remark = "已开通" where mobile = %s'
        ...
        for row in self.cursor.fetchall():
            address_id = self.get_all_area_id(row["address"])
            print(row["mobile"])
            self.cursor.execute(query_user, (row["mobile"]))
            id = self.cursor.fetchall()
            if id == ():
                print('叮当码培训学校没有此用户')
                self.cursor.execute(status_3, (row["mobile"]))
                self.content.commit()
                continue;
            user_id= id[0]
            print('用户ID：',user_id["id"],'学校：',row["school"])
            self.cursor.execute(user_role, user_id["id"])
